...
class allValidations {
    ...
    static validateBooking(req,res,next) {
        ...
        const schema = Joi.object().keys({
            trip_id: Joi.string().regex(/^[1-9]+$/).trim().required().error(new Error('Please ensure that the trip id is an integer only')),
            seat_number: Joi.string().regex(/^[1-9]+$/).trim().required().error(new Error('Please ensure that the seat number is an integer only')),
